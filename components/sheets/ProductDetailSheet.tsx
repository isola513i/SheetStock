    ...
    <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col animate-in fade-in duration-200">
      ...
      <div className="shrink-0 px-4 pb-[calc(env(safe-area-inset-bottom,0px)+12px)] pt-3">
        <p className="text-center text-white/40 text-[10px]">
          {scale > 1
            ? "แตะ 2 ครั้งเพื่อย่อ"
            : "แตะ 2 ครั้งเพื่อขยาย • ใช้ 2 นิ้วซูม"}
        </p>
      </div>
    </div>
  );
...
function DataRow({
  ...
  return (
    <div
      className={`flex justify-between items-start py-3 ${!isLast ? "border-b border-dashed border-gray-200" : ""} gap-4`}
    >
      <span className="text-gray-500 font-medium text-sm shrink-0">
        {label}
      </span>
      <span className="text-gray-900 text-sm text-right break-words overflow-hidden">
        {value}
      </span>
    </div>
  );
...
function getStockStatus(qty: number, locale: Locale) {
  if (qty <= 0)
    return {
      label: t("product.outOfStock", locale),
      color: "bg-red-100 text-red-700 border-red-200",
    };
  if (qty < 10)
    return {
      label: t("product.lowStock", locale),
      color: "bg-yellow-100 text-yellow-700 border-yellow-200",
    };
  return {
    label: t("product.inStock", locale),
...
export function ProductDetailSheet({
  ...
  const mainImageSrc = selectedItem?.imageUrl || "/icons/icon-192x192.png";
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div className="rounded-xl bg-white/16 px-3 py-2">
                    <p className="text-[11px] text-white/80">
                      {t("product.quantity", locale)}
                    </p>
                    <p className="text-white text-sm font-medium">
                      {selectedItem.quantity} {t("product.pieces", locale)}
                    </p>
                  </div>
                  <div className="rounded-xl bg-white/16 px-3 py-2">
                    <p className="text-[11px] text-white/80">
                      {t("product.price", locale)}
                    </p>
                    <p className="text-white text-sm font-medium">
                ...
                <div className="bg-white rounded-[1.6rem] p-5 border border-gray-200 shadow-sm">
                  ...
                  <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4 border border-gray-100 mb-5">
                    <div>
                      <p className="text-[11px] text-gray-500 font-medium">
                        {t("product.barcode", locale)}
                      </p>
                      <p className="text-gray-900 text-sm font-mono mt-0.5">
                        {toDisplayText(selectedItem.barcode)}
                      </p>
                    </div>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 space-y-2 border border-gray-100">
                      <DataRow
                        label={t("product.category", locale)}
                        value={toDisplayText(selectedItem.category)}
                      />
                      <DataRow
                        label={t("product.brand", locale)}
                        value={toDisplayText(selectedItem.brand)}
                      />
                      <DataRow
                        label={t("product.series", locale)}
                        value={toDisplayText(selectedItem.series)}
                      />
                      <DataRow
                        label={t("product.expiryDate", locale)}
                        value={toDisplayText(selectedItem.expiryDate)}
                        isLast
                      />
                    </div>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 space-y-2 border border-gray-100">
                      <DataRow
                        label={t("product.price", locale)}
                        value={formatCurrency(selectedItem.price)}
                      />
                      <DataRow
                        label={t("product.quantity", locale)}
                        value={`${selectedItem.quantity} ${t("product.pieces", locale)}`}
                      />
                      <DataRow
